...
";
$centers = $pdo->query($sql)->fetchAll();
...
<header class="top-header glow-card">
  <div class="welcome-text">
    ...
    <p>Live capacity of every center &mdash; updates on its own as deposits come in.</p>
  </div>

  <!-- "Live" pill. It is pure PHP: the page meta-refreshes, so this
       timestamp changing on its own proves the data is fresh. -->
  <div class="live-badge">
    <span class="live-dot"></span>
    <div class="badge-info">
      <span class="badge-label">Live</span>
      <span class="badge-points">updated <?= date('H:i:s') ?></span>
    </div>
  </div>
</header>
...
<section class="centers-grid">
  ...
  <?php if ($totalCenters === 0): ?>
    <p class="loading-text">No collection centers found.</p>
  <?php endif; ?>
  ...
  <?php foreach ($view as $c): ?>
    <div class="center-card glow-card level-<?= $c['level'] ?>">
      ...
      <!-- The full message OR the space-left message -->
      <?php if ($c['is_full']): ?>
