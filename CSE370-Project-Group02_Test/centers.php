...
/* ============================================================
   ...
   Shows every collection center with:
     - its max capacity
     - how much is already used  (= the center's stored current_capacity)
   ...
   THE KEY IDEA
   ------------
   "Used capacity" is now STORED on each collection_center row in the
   column current_capacity. Every time a manager accepts a deposit,
   deposit_data.php adds that weight to current_capacity. So this page
   just reads the number straight from the collection_center table —
   the same table the grader can open in phpMyAdmin and watch change.
   ...
   whole page every few seconds, re-reading the stored number. That is
   the pure-PHP way to be dynamic -- no fetch(), no polling script.
...
/* ------------------------------------------------------------
   1. ONE simple query gets every center AND its used weight,
      straight from the stored current_capacity column. No join,
      no SUM -- the number is already kept up to date for us.
   ------------------------------------------------------------ */
$sql = "
    SELECT  Center_ID,
            Center_name,
            Address,
            max_capacity,
            current_capacity AS used_weight
    FROM    collection_center
    WHERE   TRIM(SUBSTRING_INDEX(Address, ',', -1)) = ?
...
foreach ($centers as $c) {
    ...
    /* --------------------------------------------------------
       3. Keep the DB's Status column in step with what we just
          computed, so other pages that read collection_center.Status
          see the right value too. Only writes when the stored value
          is actually different, so refreshes don't spam the database.
       -------------------------------------------------------- */
    try {
        $upd = $pdo->prepare("
...
<section class="centers-grid">
  ...
  <?php foreach ($view as $c): ?>
    <div class="center-card glow-card level-<?= $c['level'] ?>">
      ...
      <!-- The full message OR the space-left message (display only) -->
      <?php if ($c['is_full']): ?>
