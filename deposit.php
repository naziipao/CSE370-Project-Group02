...
// The user's own city decides which centers they can deposit at.
$stmt = $pdo->prepare("SELECT City FROM `user` WHERE User_id = ?");
$stmt->execute([$user_id]);
$my_city = $stmt->fetchColumn();

// If they arrived from the Collection Centers page ("Deposit Here"),
// that center is pre-selected in the dropdown.
$preselect = $_GET['center'] ?? '';

$active   = get_user_active_request($pdo, $user_id);          // Pending one, or false
$centers  = get_centers_with_capacity($pdo, $my_city);        // only this city's centers
...
?>
      ...
      <section class="deposit-card glow-card">
        ...
        <div class="rule-note">
          &#127793; Showing collection centers in
          <strong><?= $my_city ? htmlspecialchars($my_city) : 'your city' ?></strong>.
          You earn <strong><?= POINTS_PER_KG ?> points per kg</strong> once the
          center manager accepts your waste.
        </div>

        <?php if (count($centers) === 0): ?>
          <p class="empty-msg">
            There are no collection centers in
            ...
            <select id="centerId" name="center_id" required>
              ...
              <?php foreach ($centers as $c): ?>
                <option value="<?= htmlspecialchars($c['id']) ?>"
                        <?= $c['is_full'] ? 'disabled' : '' ?>
                        <?= ($c['id'] === $preselect && !$c['is_full']) ? 'selected' : '' ?>>
