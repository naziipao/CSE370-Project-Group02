<?php
/* ============================================================
   pickup.php  -  FEATURE 1(Aurpita): Home Pickup Requests (user end)
   ...
   Updated for:
     ...
     - the phone number is shown so the user can dial it themselves
     - Pickup_ID generated safely using AUTO_INCREMENT (pickup_seq)
       to avoid race conditions when multiple users submit at once

   The page reloads itself every 15 seconds while a request is
   live, which is how it notices status changes.
   ============================================================ */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action == 'create') {
        ...
        // Server-side weight check — HTML min="10" is a hint to the
        // browser only, not a security boundary.
        ...
        /* ---- Safe ID generation using AUTO_INCREMENT ----
          insert with a temporary ID first so MySQL assigns
          the auto-increment integer (pickup_seq) atomically.
          lastInsertId() reads back YOUR connection's own value,
          never another user's. Then format and write the real ID.  */

        // Step 1: insert with placeholder ID
        ...
        $stmt->execute([$user_id, $pickup_type, $address, $city, $weight, $notes]);

        // Step 2: read the guaranteed-unique sequence number MySQL just gave us
        $seq       = $pdo->lastInsertId();
        $pickup_id = 'PU' . str_pad($seq, 2, '0', STR_PAD_LEFT);
        // gives PU01, PU02 ... PU09, PU10, PU11 ...

        // Step 3: write the formatted ID back
        $stmt = $pdo->prepare("UPDATE pickup_request SET Pickup_ID = ? WHERE pickup_seq = ?");
        $stmt->execute([$pickup_id, $seq]);
    ...
    if ($action == 'cancel') {
        ...
        // AND User_id = ? ensures a user can only cancel their own request.
        // AND status IN (...) stops cancelling something already arrived/done.
        ...
        // Subquery finds who was assigned; NULL subquery = no-op safely.
    ...
    if ($action == 'confirm') {
        ...
        // Record only the user's own flag.
        $stmt = $pdo->prepare("UPDATE pickup_request
        ...
        // Read the row back fresh to check both flags together.
        ...
        if ($req && $req['user_confirmed'] && $req['recycler_confirmed']) {
            ...
            // Both sides confirmed — close the request and award points.
            $stmt = $pdo->prepare("UPDATE pickup_request
            ...
            // Free the recycler for another job.
            $stmt = $pdo->prepare("UPDATE recycler SET is_available = 1
                                   WHERE Recycler_ID = ?");
...
// LEFT JOIN so a Pending request (no recycler yet) still shows up.
...
// Only keep auto-refreshing while something is live.
if ($active) {
    $auto_refresh = 15;
}

// Has a Pending request been waiting longer than the limit?
...
?>
      ...
      <section class="pickup-card glow-card">
        ...
        <!-- Who accepted the job. Number shown, no call button. -->
      ...
      <section class="pickup-card glow-card">
        ...
        <table class="history-table">
          <tr>
            <th>ID</th><th>Type</th><th>Weight</th>
            <th>Recycler</th><th>Date</th><th>Status</th>
          </tr>
