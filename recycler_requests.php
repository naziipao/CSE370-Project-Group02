...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action == 'accept') {
        ...
        /* Race condition fix: the WHERE clause checks both status AND
           Recycler_ID IS NULL atomically inside MySQL.
           Two recyclers clicking Accept at the same moment:
             - First  → updates 1 row, status flips to Assigned
             - Second → finds status is no longer 'Pending', updates 0 rows
           rowCount() catches this: 0 means someone else was faster. */
    ...
    if ($action == 'status') {
        ...
        // Whitelist check: $new_status comes from a hidden form field
        // which anyone can edit. Only these two values are ever valid.
        ...
        // AND Recycler_ID = ? means a recycler can only change
        // the status of a job that is assigned to them specifically.
    ...
    /* ---------- Confirm the handover (RECYCLER side) ----------
       ...
       Mirror image of the user's confirm in pickup.php.
       Points are only awarded once BOTH flags are true.
       Whoever confirms second triggers the payout. */
    ...
    if ($action == 'confirm') {
        ...
        // Set only the recycler's own flag.
        // AND Recycler_ID = ? prevents touching someone else's job.
        $stmt = $pdo->prepare("UPDATE pickup_request
        ...
        // Read both flags back fresh from the database.
        ...
        if ($req && $req['user_confirmed'] && $req['recycler_confirmed']) {
            ...
            // Both sides confirmed. Close the job and award points.
    ...
    if ($action == 'release') {
        ...
        // Setting Recycler_ID back to NULL and status back to 'Pending'
        // makes this job reappear in other recyclers' open-jobs list,
        // because that query filters on status='Pending' AND Recycler_ID IS NULL.
        // AND status IN (...) excludes 'Arrived' — once at the door, no bailing.
        $stmt = $pdo->prepare("UPDATE pickup_request
...
// --- the job this recycler is currently on ---
// LEFT JOIN phone because a user might have no phone row on file.
...
// Three filters: only Pending, only unassigned (Recycler_ID IS NULL),
// only in this recycler's own city. Oldest first — fair queue.
...
// --- jobs this recycler has finished ---
...
?>
      ...
      <header class="top-header glow-card">
        <div class="welcome-text">
          ...
          <p>Pick up any request you like in your area.</p>
        </div>
      ...
      <section class="pickup-card glow-card current-job">
        ...
        <div class="card-head">
          <h2 class="card-title">
            🔧 Your Current Job &nbsp;·&nbsp; <?= htmlspecialchars($my_job['Pickup_ID']) ?>
          </h2>
        ...
        <div class="pickup-detail-grid">
          <div class="detail-box">
            ...
            <span class="detail-value">
              <?= htmlspecialchars($my_job['FirstName'] . ' ' . $my_job['LastName']) ?>
            </span>
          </div>
          <div class="detail-box">
            <span class="detail-label">Phone (dial to coordinate)</span>
            <span class="detail-value">
              <?= $my_job['user_phone'] ? htmlspecialchars($my_job['user_phone']) : '—' ?>
            </span>
          </div>
          <div class="detail-box">
        ...
        <?php if ($my_job['status'] == 'Arrived'): ?>
          ...
          <div class="confirm-box">
            ...
            <form method="POST" action="recycler_requests.php">
              ...
              <input type="hidden" name="pickup_id" value="<?= $my_job['Pickup_ID'] ?>">
              <button type="submit" class="btn-confirm-recycler">
      ...
      <section class="pickup-card glow-card">
        ...
        <?php if ($my_job): ?>
          <p class="empty-msg">
            You are already on a job. Finish or release it to see open requests again.
          </p>

        <?php elseif (count($open_requests) == 0): ?>
          <p class="empty-msg">
            No open requests in <?= htmlspecialchars($recycler_city) ?> right now.
            <!-- ============ PAGE REFRESHES AFTER EVERY 20sec ============ -->
          </p>
      ...
      <section class="pickup-card glow-card">
        ...
        <table class="history-table">
          <tr>
            ...
            <th>Weight</th><th>Date</th><th>Status</th>
          </tr>
